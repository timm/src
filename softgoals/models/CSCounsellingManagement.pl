...
goals(soft) <--
  [ or([ considerationOfFeedbackManagers,
         ...
         callDurationConsistencyCounsellors,
         increaseFundingForTraining1,
         ...
         reduceSpendingOnEmployeesSalaries,
...
accommodateSchedulesCounsellingManagers <--
  [ hurts(effectiveScheduling) ].
accommodateSchedulesCounsellingManagers2 <-- [helps(requestShifts)].
...
advanceNoticeToCounsellorsAboutITTransitions <--
  [ hurts(facilitateFasterChangesInCounsellorDuties) ].
avoidLiabilityProblems <-- [helps(secondReadingOfWebPosts1)].
avoidRefusalOfServices <--
  [ helps(avoidRelationshipsWithSpecificCounsellor1) ].
callDurationConsistencyCounsellors <--
  [ helps(analyzeCallStatistics2) ].
continualImprovementCounsellingSkills <--
  [ helps(increaseTrainingForCounsellors),
    helps(improveTrainingForCounsellors),
    helps(feedbackToCounsellorsBeProvided1),
    helps(increaseFeedback),
    helps(improveITSkillsOfCounsellors),
    helps(supervisionBePerformedAtLeastEvery3Months1),
    helps(professionalCounsellorsBeUsed),
...
effectiveUseOfTechnologyCounsellors <--
  [ helps(positiveAttitudeTowardsITCounsellors),
    helps(improveITSkillsOfCounsellors1) ].
environmentConductiveToConcentration <--
  [ helps(professionalWorkEnvironment) ].
happinessCounsellingManager <--
  [ helps(environmentConductiveToConcentration),
    helps(accommodateSchedulesCounsellingManagers2),
    helps(easierJob),
    helps(avoidLiabilityProblems) ].
happinessCounsellingManager1 <--
  [ helps(reduceTurnoverCounsellors),
    helps(reduceTurnoverCounsellingManagement) ].
happinessCounsellors <--
  [ helps(reduceTurnoverCounsellors),
    helps(reduceTurnoverCounsellingManagement),
    hurts(reduceSpendingOnEmployeesSalaries),
    helps(increasedCounsellingResource),
    helps(positiveInternalOpinion1) ].
happinessOfCounsellors <--
  [ helps(considerCounsellorsFeedback),
    helps(increaseCommunicationBetweenCounsellorsAndSupervisors),
    hurts(callDurationConsistencyCounsellors),
    helps(supportCounsellors),
    helps(increaseNumberOfCounsellors),
    hurts(pressureCounsellorsToProvideOnlineServices1),
    helps(clearerCallClassificationCatagories1),
    helps(accommodateSchedulesCounsellors) ].
helpAsManyKidsAsPossible <--
  [ helps(avoidRefusalOfServices),
    helps(bilingualCounsellorsBeHired2),
    helps(effectiveScheduling1) ].
highQualityCounselling <--
  [ helps(continualImprovementCounsellingSkills1),
    helps(improvedQualityAssuranceCounsellingReviewsAndEvaluation),
    helps(callDurationConsistencyCounsellors),
    helps(supportCounsellors),
    helps(happinessOfCounsellors),
    helps(qualifiedCounsellors),
...
highQualityOnlineCounselling <--
  [ helps(effectiveUseOfTechnologyCounsellors1),
    helps(improvedWritingSkillsCounsellors2) ].
improveHiringProcessOfCounsellors <--
  [ helps(improveJobDescriptionsCounsellors),
    helps(increasedEmphasisOnITInHiringProcessOfCounsellors2) ].
improveITSkillsOfCounsellors <--
  [ helps(increaseCounsellorsExperienceWithTechnology),
    helps(increasedEmphasisOnITInHiringProcessOfCounsellors1),
    helps(increaseITTrainingForCounsellors1) ].
improveITSkillsOfCounsellors1 <-- [helps(putOnWebModeratorMeetings)].
improveQualityAssuranceTechnology <--
  [ helps(performanceReviewInformationBeCollectedInDataBase),
    helps(callsBeRecordedIntoADataBase),
    helps(improveCallRecordingEquipment1) ].
improveTrainingForCounsellors <--
  [ helps(aTrainingRoomBePresent),
    helps(aTrainingProgramBePresent) ].
improvedQualityAssuranceCounsellingReviewsAndEvaluation1 <--
  [ helps(improveQualityAssuranceTechnology),
    helps(rigorousEmployeeEvaluations) ].
increaseCommunicationBetweenCounsellorsAndSupervisors <--
  [ helps(considerCounsellorsFeedback),
    helps(counsellorsBeKeptInformed),
    helps(debriefWithCounsellors),
    helps(attendPartTimeMeetings) ].
increaseEnthusiasmCounsellorsForIT <--
  [ helps(increasedUnderstandingOfPurposesOfITCounsellors),
    hurts(supportAnalysisCounsellorsTime) ].
increaseFeedback <-- [helps(increaseITMethodsToAcquireFeedback1)].
increaseITTrainingForCounsellors1 <--
  [ helps(anITTrainerBePresent),
    helps(aTrainingComputerBePresent) ].
increaseNumberOfCounsellors2 <--
  [ helps(increasedCounsellingResource) ].
increaseTrainingForCounsellors <--
  [ helps(increaseFundingForTraining1),
    helps(aTrainingProgramBePresent),
    helps(increaseITTrainingForCounsellors1),
    helps(aTrainingRoomBePresent) ].
increasedCounsellingResource <-- [helps(reduceCostOfCounsellingHR2)].
increasedUnderstandingOfPurposesOfITCounsellors <--
  [ helps(clearCommunicationAboutThePurposeOfITToCounselors) ].
positiveAttitudeTowardsITCounsellors <--
  [ helps(increaseEnthusiasmCounsellorsForIT),
    helps(reduceResistanceCounsellorsForIT) ].
provideOnlineServicesAllCounsellors <--
  [ helps(pressureCounsellorsToProvideOnlineServices),
    helps(positiveAttitudeTowardsITCounsellors),
    helps(improveITSkillsOfCounsellors1),
    helps(facilitateFasterChangesInCounsellorDuties) ].
qualifiedCounsellors <--
  [ helps(improvedQualityAssuranceCounsellingReviewsAndEvaluation),
    helps(improveHiringProcessOfCounsellors1) ].
reduceCostOfCounsellingHR <-- [helps(analyzeCallStatistics)].
reduceCostOfCounsellingHR2 <--
  [ hurts(increaseNumberOfCounsellors2),
    hurts(bilingualCounsellorsBeHired),
    helps(reduceSpendingOnEmployeesSalaries) ].
reduceResistanceCounsellorsForIT <--
  [ helps(advanceNoticeToCounsellorsAboutITTransitions),
    helps(increasedUnderstandingOfPurposesOfITCounsellors),
    hurts(supportAnalysisCounsellorsTime) ].
reduceStaggeringOfSchedules1 <-- [hurts(effectiveScheduling)].
reduceTurnoverCounsellingManagement <--
  [ helps(happinessCounsellingManager1) ].
reduceTurnoverCounsellors <-- [helps(happinessCounsellors)].
sufficientCounsellingResources1 <--
  [ helps(increasedCounsellingResource) ].
sufficientCounsellingResources2 <-- [helps(effectiveScheduling)].
supportCounsellors <--
  [ helps(provideOneOnOneSupportToCounsellors),
    helps(supportDayToDayOnTheFloorNeedsOfCounsellors),
    helps(supportDayToDayClinicalNeedsOfCounsellors2),
    helps(debriefWithCounsellors) ].
supportDayToDayClinicalNeedsOfCounsellors1 <--
  [ helps(improveQualityAssuranceTechnology),
    helps(feedbackToCounsellorsBeProvided),
    helps(provideFeedbackOnWebPosts) ].
