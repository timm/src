% CSFDandMarketing  (nfr3 dialect: <-- rules, soft bodies are edges)
:- discontiguous (<--)/2.
:- dynamic (<--)/2.
goals(hard) <--
  [ counselorsAttendFundraisingEvents,
    contributeToACause,
    receiptBeProvided,
    pledgesBeCollected,
    projectDeadlinesBeMet1,
    sponsorsForEventsBeFound,
    marketingFromGovernmentBeAcquired,
...
goals(soft) <--
  [ or([ sponsorshipBeBeneficial,
         ...
         longTermFunding,
         improveImageToKids,
         acquireGovernmentFunding,
         marketFundraisingEvents1,
         engageEmployeesInFundraisingEvents,
         meetCorporateSponsorsNeeds,
         trustOfSponsorsTowardsCS2,
         contributeToAGoodCause,
         increaseVolunteers1,
         ...
         regionalStaffFeelIncluded1,
         recognizeNSponsorContribution,
         qualityServices,
         responsibleUsageSponsorFunds2,
         ...
         followHighestEthicalGuidelines1,
         contributeToAGoodCause1,
         trustOfSponsorsTowardsCS1,
         increasePhilanthropicDonations,
         increasedInteractionCounselorsWithMedia,
         sponsorPartnerNcontacts1,
         provincialGovernment,
         responsibleUsageSponsorFunds1,
         minimizeExpenses,
         ...
         followHighestEthicalGuidelines2,
         engageEmployeesInEvents2,
         demonstrableServices1,
         ...
         retainSponsors,
         experiencedMarketingPartners2,
         demonstrableServices2,
...
acquireFreeServices <--
  [ helps(attractSponsors2),
    helps(sponsorPartnerNcontacts2) ].
agreementStaffOverCorporatePartnerExpectations <--
  [ helps(periodicalCommunicationWithRegionalStaffBeMaintained),
    helps(getCorporatePartnerInformationFromDL) ].
appearAsGoodCorporateCitizens <--
  [ helps(recognition),
    helps(recognition),
    helps(cobranding) ].
attractEventParticipants <--
  [ helps(increaseNAwareness),
    helps(eventsBeMarketed),
    helps(positiveReputation) ].
attractSponsors <--
  [ helps(increasedAvailabilityCounselorsToPublicSponsors),
    helps(sponsorPartnerNcontacts1) ].
attractSponsors2 <--
  [ helps(useSponsorLogo),
    helps(goodMediaExposure),
    helps(sponsorPartnerNcontacts2),
    helps(sponsorPartnerNcontacts2),
    helps(successfulMarketingCampaign) ].
availabilityForAdministrationAndAccountingPurposesAcrossCS <--
  [ helps(frontEndForIncomeRecordingBeSupported),
    helps(donorInformationBeRecorded) ].
avoidOverMarketingServices <-- [makes(marketOnlyForSeriousIssues)].
cobranding <--
  [ helps(provideLogoToCS),
    makes(becomeAOfficialPartner) ].
commitmentToCSCauseFromSponsorsEmployees <--
  [ helps(engageEmployeesInFundraisingEvents),
    helps(engageEmployeesInPromotingAwareness) ].
contributeToAGoodCause <-- [helps(trustOfSponsorsTowardsCS)].
contributeToAGoodCause1 <-- [helps(trustOfDonor)].
corporateSponsorFeelIncluded <--
  [ helps(upToDateInformationOnPrograms),
    helps(quickResponseToQuestionsAndConcerns) ].
corporateSponsorsFeelIncluded <--
  [ makes(meetCorporateSponsorsNeeds),
    helps(informationBeSharedWithSponsors) ].
engagementOfSponsorEmployeesInFundDevelopment <--
  [ helps(attractEventParticipants),
    helps(increaseInvolvementSponsorsInPuttingOnEvents) ].
engagementSponsorsInTheCauseOfCS <--
  [ helps(meetCorporateSponsorsNeeds),
    helps(commitmentToCSCauseFromSponsorsEmployees) ].
exclusiveRelationshipsOfficialPartners <--
  [ makes(becomeAOfficialPartner) ].
getFreeAdvertisement <-- [helps(sponsorPartnerNcontacts2)].
goodMediaExposure <--
  [ helps(answersBeProvidedToMediaQueries),
    helps(proactiveMediaRelationship) ].
goodProjectManagement <-- [helps(manageProjects)].
goodProjectManagement2 <-- [helps(manageNprojects)].
happinessStaff <--
  [ helps(regionalStaffFeelIncluded1),
    helps(conflictsBeManaged),
    helps(payFundDevelopmentAndMarketingStaff),
    helps(positiveInternalOpinion),
    helps(staffBeSupported) ].
highResponseIndividualDonors <--
  [ helps(increaseConnectionCounselorsAndCommunity) ].
highResponseIndividualDonors2 <--
  [ helps(increasedInvolvementCounselorsInEvents1) ].
improveImageToKids <--
  [ helps(saSpeakAtFundraisers),
    helps(saGiveCSPresentations),
    helps(developExternalComunications),
    helps(successfulMarketingCampaign) ].
increaseAwareness <--
  [ helps(marketPHLBrand),
    helps(saSpeakAtFundraisers),
    helps(goodMediaExposure),
    helps(saGiveCSPresentations) ].
increaseConnectionCounselorsAndCommunity <--
  [ helps(increasedInvolvementCounselorsInEvents) ].
increaseFreeServices <-- [helps(sponsorPartnerNcontacts1)].
increaseInvolvementSponsorsInPuttingOnEvents <--
  [ makes(sponsorsForEventsBeFound),
    helps(sponsorRelationshipsBeManaged) ].
increasePhilanthropicDonations <--
  [ helps(highResponseIndividualDonors2),
    helps(getPhilanthropicDonations) ].
increaseVolunteers <--
  [ helps(createLifeLongVolunteerSpirit1),
    helps(uniteVolunteersAndSponsors) ].
increasedAvailabilityCounselorsToPublicSponsors <--
  [ helps(increasedInvolvementCounselorsInEvents1) ].
increasedCorporateSponsorFunds <--
  [ helps(getCorporateSponsorFunds),
    helps(attractSponsors),
    helps(increaseInvolvementSponsorsInPuttingOnEvents) ].
increasedInteractionCounselorsWithMedia <--
  [ helps(counselorsWhoWantAPublicVoiceBeTrained),
    helps(counselorSpeakOnKidsIssuesInGeneral) ].
increasedInvolvementCounselorsInEvents <--
  [ helps(counselorSpeakOnKidsIssuesInGeneral) ].
increasedInvolvementCounselorsInEvents1 <--
  [ helps(counselorsAttendFundraisingEvents) ].
inspiredStaff <--
  [ helps(increasedInvolvementCounselorsInEvents1),
    helps(elaborateFundraisingTargets),
    helps(happinessStaff3) ].
longTermFunding <-- [helps(retainSponsors)].
marketCSBrand <--
  [ helps(engagementSponsorEmployeesInPromotingAwareness),
    helps(successfulMarketingCampaign),
    helps(experiencedMarketingPartners),
    helps(sponsorsMarketingChannelsBeUsed),
    helps(positiveReputationOfCS2) ].
marketFundraisingEvents1 <--
  [ helps(saSpeakAtFundraisers),
    helps(proactiveMediaRelationship),
    helps(developExternalComunications),
    helps(projectDeadlinesBeMet1) ].
marketPHLBrand <--
  [ helps(getFreeAdvertisement),
    helps(successfulMarketingCampaign),
    helps(experiencedMarketingPartners),
    helps(sponsorsMarketingChannelsBeUsed),
    helps(positiveReputationOfCS2) ].
matchFundraisingTargets <--
  [ helps(retainSponsors),
    helps(increasePhilanthropicDonations),
    helps(pledgesBeCollected),
    helps(increaseInvolvementSponsorsInPuttingOnEvents),
    helps(getCorporateSponsorFunds),
    helps(increasedCorporateSponsorFunds),
    helps(getPhilanthropicDonations),
    helps(acquireGovernmentFunding),
    helps(fundsBeRaised) ].
meetCorporateSponsorsNeeds <--
  [ helps(exclusiveRelationshipsOfficialPartners2),
    helps(corporatePartnerExpectationsBeSharedWithStaff),
    helps(quickResponseToSponsors),
    helps(recognizeNSponsorContribution1) ].
minimizeCostOfEvents <--
  [ helps(increaseInvolvementSponsorsInPuttingOnEvents),
    helps(maintain20CostsAgainstRevenuesRatio),
    helps(increaseFreeServices) ].
minimizeExpenses <--
  [ helps(acquireFreeServices),
    helps(attractSponsors2),
    helps(marketingFromGovernmentBeAcquired),
    helps(sponsorsMarketingChannelsBeUsed),
    helps(getFreeAdvertisement),
    hurts(developExternalComunications) ].
positiveAssociationNByConsumers <--
  [ helps(cobranding),
    helps(recognition),
    helps(credibilityCSBrand),
    helps(contributeToAGoodCause) ].
positiveImageToEmployees <--
  [ helps(engageEmployeesInEvents2),
    helps(contributeToAGoodCause) ].
positiveReputation <-- [helps(singleCharitableRegistrationNumber1)].
positiveReputationOfCS2 <--
  [ helps(useSponsorLogo),
    helps(successfulMarketingCampaign),
    helps(involvementNotForProfitPartnersInEvents) ].
presentProposalConvincinglyToSponsors <-- [helps(presentationSkills)].
proactiveMediaRelationship <--
  [ helps(increasedInteractionCounselorsWithMedia),
    helps(marketingBeMadeUsingDedicatedCounselor) ].
reachMoreKidsSponsors <-- [helps(recognition), helps(cobranding)].
recognition <--
  [ helps(contributeToAGoodCause),
    helps(contributeToACause) ].
recognizeNSponsorContribution2 <--
  [ helps(createThankYouAds),
    helps(grantExclusiveUseOfBrandAndLogo),
    helps(sendRecognitionLetters) ].
reduceInternalCommunicationConfusion <--
  [ helps(agreementStaffOverCorporatePartnerExpectations) ].
regionalStaffFeelIncluded <--
  [ helps(periodicalCommunicationWithRegionalStaffBeMaintained),
    helps(workWithTheRegionsToImplementEvents) ].
retainSponsors <--
  [ helps(sponsorRelationshipsBeManaged),
    helps(positiveReputation),
    helps(increasedAvailabilityCounselorsToPublicSponsors),
    helps(increaseNAwareness),
    helps(corporatePartnerFundraisingExpectationsBeKnown),
    helps(successfulEvents) ].
retainSponsors1 <--
  [ helps(trustOfSponsorsTowardsCS2),
    helps(companyRelationshipBeRetainedAfterMergers),
    helps(upToDateCorporatePartnerInformation),
    helps(corporateSponsorsFeelIncluded),
    helps(commitmentToCSCauseFromSponsorsEmployees) ].
sponsorshipBeBeneficial <--
  [ helps(reachMoreKidsSponsors),
    helps(positiveImageToEmployees),
    helps(appearAsGoodCorporateCitizens) ].
successfulEvents <--
  [ helps(attractSponsors),
    helps(timelyProvisionOfFreeServices1),
    helps(projectDeadlinesBeMet),
    helps(increaseVolunteers),
    helps(increaseInvolvementSponsorsInPuttingOnEvents),
    helps(uniteVolunteersAndSponsors),
    helps(attractEventParticipants),
    helps(engagementOfSponsorEmployeesInFundDevelopment),
    helps(involveNotForProfitPartnersInEvents),
    helps(fundsBeRaised),
    helps(happinessStaff3),
    helps(goodProjectManagement2) ].
successfulMarketingCampaign <--
  [ helps(goodProjectManagement),
    helps(happinessStaff1),
    helps(experiencedMarketingPartners),
    helps(projectDeadlinesBeMet1),
    helps(developExternalComunications),
    helps(corporatePartnerMarketingExpectationsBeKnown) ].
trustOfDonor <--
  [ helps(demonstrableServices2),
    helps(credibilityCSBrand2),
    helps(receiptBeProvided),
    helps(accountabilityOfServices2),
    helps(hearCSStories),
    helps(responsibleUsageSponsorFunds2),
    helps(followHighestEthicalGuidelines2) ].
trustOfSponsorsTowardsCS <--
  [ helps(accountabilityOfServices),
    helps(followHighestEthicalGuidelines),
    helps(credibilityCSBrand),
    helps(qualityServices2),
    helps(demonstrableServices),
    helps(responsibleUsageSponsorFunds) ].
uniteVolunteersAndSponsors <--
  [ helps(attractEventParticipants),
    helps(increaseInvolvementSponsorsInPuttingOnEvents) ].
upToDateCorporatePartnerInformation <--
  [ makes(informationBeSharedWithSponsors) ].
upToDateInformationOnCorporateSponsors <--
  [ helps(communicateThroughInternetAndPhone),
    helps(biWeeklyConferenceCalls) ].
