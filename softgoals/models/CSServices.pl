% CSServices  (nfr3 dialect: <-- rules, soft bodies are edges)
:- discontiguous (<--)/2.
:- dynamic (<--)/2.
goals(hard) <--
  [ answer80OfCallsWithin30SecondsOfMessageEnding,
    anonymousTechnologyBeUsed,
    servicesBeBilingual1,
    counsellorsBeProfessionallyTrained,
    nonRealTimeECounsellingBeUsed,
    servicesBeBilingual,
    nonAnonymousTechnologyBeUsed,
    ...
    webSiteContentBeUpdatedDaily,
    realTimeECounsellingBeUsed,
...
goals(soft) <--
  [ or([ decreaseResponseTIme1,
         ...
         increasePhoneResources2,
         efficientServices1,
         friendlyWebSite,
         confidentialityParents5,
         confidentialityKids5,
         availableServices1,
         relevanceInKidsLives,
         maintainServicesAboveCompetition,
         anonymityCounsellors,
         connectBackToTheCommunity2,
         connectToOtherKids,
         immediacy3,
         directResponseToKids1,
         availableServices,
         controlWebServices,
         improveWebsiteContent,
         efficientPhoneServices2,
         decreasePhoneWaitingTime,
         improveWebsiteContent2,
         ...
         confidentialityParents1,
         anonymityParents2,
         efficientWebServices,
         confidentiality,
         decreaseResponseTIme3,
         decreaseResponseTIme4,
         ...
         avoidBadAdvice4,
         increaseResourcesServices1,
         controlWebServices1,
         anonymityCounsellors1,
         reduceNumberOfStepsWebPostingTechnology1,
         friendlyWebSite2,
         increaseWebResources,
         increasePhoneResources1,
         highQualityServices,
...
implementCyberCafPortalChatRoom <--
  [ provideWrittenCounselling,
    moderateAChat,
    implementDelay ].
implementCyberCafPortalChatRoom1 <--
  [ implementCyberCafPortalChatRoom ].
...
accommodatePhoneTraffic <--
  [ helps(sufficientCounsellingResources2),
    helps(increasePhoneResources1) ].
accommodateWebSiteTraffic <--
  [ helps(webServicesSelfServe),
    makes(sufficientCounsellingResources) ].
anonymity <--
  [ helps(anonymityParents),
    helps(anonymityKids),
    hurts(acquireFeedback) ].
anonymityCounsellors <--
  [ helps(kidsUseBulletinBoardWithReplies),
    hurts(kidsUseVideoCounselling),
    helps(kidsUseOneOnOneChatRooms),
    hurts(kidsUseEmailCounselling),
    hurts(directResponseToKids),
    helps(kidsUseBulletinBoardWithDelayedModeration),
    helps(kidsUseTextMessaging),
    helps(kidsUseAskACounsellorSection),
    helps(kidsUseCyberCafPortalChatRoom) ].
anonymityKids3 <--
  [ breaks(nonAnonymousTechnologyBeUsed),
    makes(anonymousTechnologyBeUsed) ].
anonymityKids5 <-- [makes(kidsUsePhoneCounselling)].
anonymityParents2 <-- [breaks(traceCalls)].
...
avoidBadAdvice <--
  [ helps(avoidPresenceOfPedofiles),
    helps(nonRealTimeECounsellingBeUsed),
    hurts(realTimeECounsellingBeUsed),
    hurts(technologyThatAllowsDialoguesBetweenKidsBeUsed),
    helps(technologyThatDoesNotAllowDialoguesBetweenKidsBeUsed),
    helps(avoidDialoguesBetweenKids) ].
avoidBadAdvice3 <--
  [ hurts(parentsUseNToolToAllowParentsToTalkToEachOther) ].
avoidDialoguesBetweenKids <--
  [ makes(technologyThatDoesNotAllowDialoguesBetweenKidsBeUsed),
    breaks(technologyThatAllowsDialoguesBetweenKidsBeUsed) ].
avoidDialoguesBetweenKidsAndCounsellorsOnTheWeb <--
  [ hurts(kidsUseVideoCounselling),
    hurts(kidsUseBulletinBoardWithReplies),
    hurts(kidsUseBulletinBoardWithDelayedModeration),
    hurts(kidsUseCyberCafPortalChatRoom),
    hurts(kidsUseTextMessaging),
    hurts(kidsUseEmailCounselling),
    hurts(kidsUseVoiceCounselling),
    helps(kidsUseAskACounsellorSection),
    hurts(kidsUseOneOnOneChatRooms) ].
avoidPresenceOfPedofiles <--
  [ helps(controlWebServices),
    breaks(kidsUseBulletinBoardWithDelayedModeration) ].
confidentiality <--
  [ hurts(acquireFeedback),
    helps(confidentialityParents),
    helps(confidentialityKids5) ].
confidentialityKids1 <--
  [ makes(confidentialTechnologyBeUsed),
    breaks(nonConfidentialTechnologyBeUsed) ].
confidentialityKids6 <-- [helps(kidsUsePhoneCounselling)].
confidentialityParents2 <-- [hurts(traceCalls)].
confidentialityParents4 <--
  [ breaks(parentsUseBulletinBoardWithReplies) ].
...
connectBackToTheCommunity3 <--
  [ helps(parentsUseNToolToAllowParentsToTalkToEachOther),
    helps(parentsUseInformationSection),
    helps(parentsUseBulletinBoardWithReplies) ].
connectBackToTheCommunity4 <--
  [ helps(kidsUseOnlineInformationProvided) ].
controlOfCounsellingWork1 <-- [hurts(moderatedECounsellingBeUsed)].
controlWebServices <--
  [ hurts(anonymousTechnologyBeUsed),
    helps(technologyThatDoesNotAllowDialoguesBetweenKidsBeUsed),
    hurts(realTimeECounsellingBeUsed),
    hurts(kidsUseBulletinBoardWithDelayedModeration),
    helps(nonRealTimeECounsellingBeUsed),
    helps(nonAnonymousTechnologyBeUsed),
    hurts(technologyThatAllowsDialoguesBetweenKidsBeUsed) ].
controlWebServices1 <-- [helps(sufficientCounsellingResources)].
correctInterpretationOfCounsel <--
  [ hurts(kidsUseBulletinBoardWithDelayedModeration),
    ...
    hurts(kidsUseCyberCafPortalChatRoom),
    helps(kidsUseVideoCounselling),
    hurts(kidsUseTextMessaging),
    hurts(kidsUseBulletinBoardWithReplies) ].
decreasePhoneWaitingTime2 <--
  [ helps(answer80OfCallsWithin30SecondsOfMessageEnding) ].
decreaseResponseTIme <--
  [ makes(realTimeECounsellingBeUsed),
    hurts(nonRealTimeECounsellingBeUsed) ].
decreaseResponseTIme2 <-- [helps(sufficientCounsellingResources)].
directResponseToKids <--
  [ breaks(kidsUseBulletinBoardWithDelayedModeration),
    helps(easierAccessToPostReply),
    makes(kidsUseEmailCounselling),
    makes(realTimeECounsellingBeUsed),
    breaks(kidsUseAskACounsellorSection),
    breaks(kidsUseBulletinBoardWithReplies) ].
...
efficientServices1 <--
  [ helps(efficientPhoneServices),
    helps(serviceLevelsBeMet),
    helps(decreaseResponseTIme4) ].
efficientWebServices2 <--
  [ helps(easierNavigationCSWebServices2),
    helps(accommodateWebSiteTraffic) ].
empoweringKidsToHelpThemselves <--
  [ helps(kidsUseOnlineInformationProvided) ].
encourageKidsUsingWebServicesToUsePhoneServices1 <--
  [ helps(avoidDialoguesBetweenKidsAndCounsellorsOnTheWeb) ].
friendlyWebSite12 <-- [helps(immediacy6)].
friendlyWebSite2 <--
  [ helps(easierNavigationCSWebServices1),
    helps(immediacy7),
    helps(increasedWebServices4),
    helps(directResponseToKids) ].
highQualityServices <--
  [ helps(serviceLevelsBeMet),
    helps(counsellorsBeProfessionallyTrained),
    helps(relevanceInKidsLives),
    helps(anonymity),
    helps(connectBackToTheCommunity9),
    helps(confidentialityKids5),
    helps(efficientServices1),
    helps(immediacy),
    helps(avoidBadAdvice4),
    helps(kidsHaveOwnershipOfServices5),
    helps(improveWebsiteContent2),
    helps(avoidPresenceOfPedofiles2),
    helps(directResponseToKids2),
    helps(sufficientlyModeratedWebServices) ].
immediacy2 <-- [helps(maintainPhoneServices)].
immediacy6 <--
  [ helps(parentsUseNToolToAllowParentsToTalkToEachOther),
    hurts(parentsUseBulletinBoardWithReplies) ].
immediacy7 <--
  [ makes(realTimeECounsellingBeUsed),
    helps(decreaseResponseTIme),
    hurts(nonRealTimeECounsellingBeUsed) ].
improveImageToKids <--
  [ helps(anonymityKids3),
    helps(confidentialityKids1),
    helps(avoidPresenceOfPedofiles),
    helps(decreaseResponseTIme),
    helps(relevanceInKidsLives2),
    helps(maintainServicesAboveCompetition1),
    helps(directResponseToKids),
    helps(moderatedECounsellingBeUsed),
    helps(kidsUseOnlineInformationProvided),
    helps(easierAccessToPostReply) ].
improveWebsiteContent1 <--
  [ helps(webSiteContentBeUpdatedDaily),
    helps(increaseWebResources) ].
increaseNumberOfServices <-- [helps(increasedWebServices)].
increaseResourcesServices <--
  [ helps(increaseWebResources2),
    helps(increasePhoneResources) ].
increaseWebResources <--
  [ helps(onlyOnlineRequestFromCanadiansAccepted) ].
increasedWebServices4 <--
  [ helps(kidsUseTextMessaging),
    helps(kidsUseEmailCounselling),
    helps(kidsUseOneOnOneChatRooms),
    helps(kidsUseAskACounsellorSection),
    helps(kidsUseBulletinBoardWithDelayedModeration),
    helps(kidsUseVoiceCounselling),
    ...
    helps(kidsUseBulletinBoardWithReplies) ].
kidsHaveOwnershipOfServices <--
  [ helps(informKidsAboutAnonymityKidsOfWebServices1),
    helps(moderatedECounsellingBeUsed) ].
kidsHaveOwnershipOfServices1 <-- [makes(kidsUsePhoneCounselling)].
maintainServicesAboveCompetition1 <--
  [ helps(moderatedECounsellingBeUsed) ].
measureSuccessOfServices1 <-- [helps(implementFeedbackForm)].
measureSuccessOfServices2 <-- [helps(implementPhoneFeedback)].
measureSuccessOfServices4 <--
  [ helps(increaseEmphasisOnOnlineFeedbackForm1) ].
reduceContagionEffectOfHarmfulActions <--
  [ hurts(kidsUseBulletinBoardWithDelayedModeration),
    helps(implementDelay),
    helps(effectiveChatRoomFilters),
    helps(avoidDialoguesBetweenKids),
    helps(scheduleChatAtSpecificTimes) ].
reduceNumberOfStepsWebPostingTechnology1 <--
  [ helps(kidsUseBulletinBoardWithDelayedModeration),
    hurts(kidsUseBulletinBoardWithReplies),
    hurts(maintainAskACounsellorSection) ].
reducePrankCalls1 <--
  [ hurts(confidentialityKids),
    helps(implementAntiPrankingMessage) ].
relevanceInKidsLives <-- [helps(highQualityServices)].
similarityOfProblems <--
  [ hurts(technologyThatDoesNotAllowDialoguesBetweenKidsBeUsed),
    helps(kidsReadPollsAboutKids),
    helps(technologyThatAllowsDialoguesBetweenKidsBeUsed),
    helps(kidsReadGeneralQuestionsAndAnswers),
    helps(kidsReadGetInformedSectionOfWebSite) ].
similarityWithOtherParentsProblems <--
  [ helps(parentsUseBulletinBoardWithReplies),
    helps(parentsUseNToolToAllowParentsToTalkToEachOther) ].
sufficientlyModeratedWebServices2 <--
  [ helps(sufficientCounsellingResources) ].
sufficientlyModeratedWebServices4 <--
  [ hurts(kidsUseBulletinBoardWithDelayedModeration) ].
webServicesSelfServe2 <--
  [ helps(kidsReadGetInformedSectionOfWebSite),
