;;;; models/KidsandYouth.lisp : GENERATED from models/KidsandYouth.py by to-lisp.py
...
(<- comfortablenessWithService (and (helps ownershipOfServiceKids) (helps supportAndBeSupportedByOtherKids) (helps servicesBeFree1) (helps anonymityService) (helps similarityWithOtherKidsProblems1) (helps beInformedOfServiceAnonymity) (hurts provideFeedback) (helps confidentialityService) (helps patientCounselor1)))
(<- effectiveHelpInCrisis (and (helps decreasePhoneWaitingTime) (helps immediacyService)))
(<- effectiveHelpInNonCrisisSituation (helps immediacyService))
(<- getEffectiveHelp (and (helps patientCounselor1) (helps effectiveHelpInCrisis) (helps highQualityService) (helps availabilityService) (helps easyAccessToPostReply) (helps friendlyWebSite) (helps ventEmotions) (helps effectiveHelpInNonCrisisSituation)))
(<- privacy (and (hurts provideFeedback) (helps childrenDecideWhenToHangUpAndCall) (helps confidentialityService) (helps anonymityService)))
(<- safetyOfServiceUsage (and (helps confidentialityService) (helps anonymityService) (helps immediacyService)))
(<- supportAndBeSupportedByOtherKids (helps connectWithOtherKids))
(<- ventEmotions (helps patientCounselor1))
(defparameter *hard* '(helpBeAcquired servicesBeFree informationBeAcquiredOnWebsite servicesBeFree1 beInformedOfServiceAnonymity))
