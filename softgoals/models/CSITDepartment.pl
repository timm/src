% CSITDepartment  (nfr3 dialect: <-- rules, soft bodies are edges)
:- discontiguous (<--)/2.
:- dynamic (<--)/2.
goals(hard) <--
...
goals(soft) <--
  [ or, easilyAccessableTechnologyInstructions1,
         ...
         adequatelyCustomizableCSTechnology,
         simpleTechnology,
         ...
         adjustToSoftwareChanges,
         considerationOfFeedbackITProviders,
         increaseITResources1,
         expandITDepartment,
...
costEffectiveTechnology <--
  [ helps(hardwareBeAcquiredForFree),
    helps(acquireSoftwareForFree),
    hurts(payForHardware),
    hurts(useT1),
    hurts(payForSoftware),
    helps(useCurrentMethod),
    helps(useOracleForDataBase),
    helps(acquireWebServerForFree),
    hurts(payForWebServer) ].
costEffectiveTraining <--
  [ hurts(aTrainingComputerBePresent1),
    helps(useInformalBuddySystemForTraining),
    hurts(anITTrainerBePresent1) ].
effectiveITTraining <--
  [ helps(increasedEmphasisOnITInHiringProcessOfCounsellors1),
    helps(aTrainingComputerBePresent1),
    helps(itDeptProvidesOrientationAndTraining),
    helps(increaseITTrainingResources) ].
expandITDepartment <--
  [ makes(increaseITResources),
    helps(costEffectiveTechnology) ].
fitBetweenSystemCapabilitiesAndCSRequirements <--
  [ helps(adequatelyCustomizableCSTechnology) ].
improveQualityAssuranceTechnology1 <--
  [ helps(increaseITMethodsToAcquireFeedback1),
    helps(performanceReviewInformationBeCollectedInDataBase1),
    helps(callsBeRecordedIntoADataBase1),
    helps(improveCallRecordingEquipment1) ].
increaseAccessSpeedRegionalOfficesToDL1 <-- [makes(useT1)].
increaseITMethodsToAcquireFeedback1 <--
  [ helps(feedbackFormBeImplemented) ].
increaseITResources <-- [helps(costEffectiveTraining)].
increaseITTrainingForCounsellors1 <--
  [ helps(increaseITTrainingResources),
    helps(aTrainingComputerBePresent1),
    helps(anITTrainerBePresent1) ].
increaseITTrainingResources <--
  [ helps(expandITDepartment),
    helps(increaseITResources),
    helps(costEffectiveTechnology) ].
keepUpWithNewTechnology <--
  [ helps(increaseITResources),
    helps(itBeUpgraded),
    hurts(csBeNotForProfit) ].
properlyAndSuitablyEquippedInITToAccomplishCSNeeds <--
  [ helps(properlyAndSuitablyEquippedInITToAccomplishFundraisingNeeds),
    helps(effectiveITTraining),
    helps(acquireITResources),
    helps(fitBetweenSystemCapabilitiesAndCSRequirements),
    helps(adjustToSoftwareChanges),
    helps(properlyAndSuitablyEquippedInITToAccomplishServiceGoals),
    helps(itBeUpgraded),
    helps(sensitiveToTheEnvironmentITEquipmentWillBeIntroducedTo),
    helps(keepUpWithNewTechnology) ].
properlyAndSuitablyEquippedInITToAccomplishFundraisingNeeds <--
  [ helps(provideDocumentLibrarySystem1),
    helps(manageDonorAccountingDataBase) ].
properlyAndSuitablyEquippedInITToAccomplishServiceGoals <--
  [ helps(implementEmailForCounsellors),
    helps(installDesktopSoftware),
    hurts(implementDirectorEnterpriseThroughBluePumpkin),
    helps(implementBulletinBoard),
    hurts(implementBulletinBoard),
    helps(implementActivityManager),
    hurts(implementActivityManager),
    helps(implementCategorizationTool),
    helps(considerationOfFeedbackFromCounsellors),
    helps(implementPhoneFeedback1) ].
sensitiveToTheEnvironmentITEquipmentWillBeIntroducedTo <--
  [ helps(itProvidersHaveKnowledgeOfCounselling),
    helps(itProvidersHaveKnowledgeOfFundraisingAndMarketing) ].
simpleTechnology1 <-- [helps(integrateITSystems)].
supportAnalysisCounsellorsTime1 <--
  [ helps(implementDirectorEnterpriseThroughBluePumpkin),
    helps(implementActivityManager) ].
