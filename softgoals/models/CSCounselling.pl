% CSCounselling  (nfr3 dialect: <-- rules, soft bodies are edges)
:- discontiguous (<--)/2.
:- dynamic (<--)/2.
goals(hard) <--
  [ cuttingAndPastingBeAddedToWebPostingSoftware,
    getRidOfUnion,
    informationResourcesBeFound,
    beConsultedAboutNewPolicies,
    toolsToBurnNegativeEnergyBePresent,
    counsellorsBeUnionized,
    allowUserToLogIn1,
    itSystemsBeIntegrated1,
    trainingBeReceived2,
    clinicalSupervisorsListenToCalls,
    aResolutionFocusApproachBeUsed,
    feedbackOnCallsBeGiven,
    iconsForCounsellingInformationBeOnDesktop,
    kidsBePreparedForFormalAbuseReportingProcess,
    counsellingPoliciesBeFollowed,
    passProbationWithinSixMonths,
    situationBeDealtWith,
    itSystemsBeIntegrated,
    beAMentorToNewCounsellors,
    counsellingPoliciesBeFollowed1,
    situationBeDealtWith1,
    briefTherapyCounsellingBeUsed,
    webPostsBeReviewedByClinicalSupervisor,
    allowUserToLogIn,
    trainingBeReceived,
...
goals(soft) <--
  [ or([ considerationOfFeedbackManagers,
         ...
         moveOnToBetterJob,
         avoidPressureToPerformDutiesQuickly,
         patientCounselor,
         increaseFeedback,
         helpAsManyKidsAsPossible2,
         simpleTechnology1,
         recognition2,
         currentResourceLinks2,
         increasedPeerSupport1,
         accommodateSchedules,
         accessibleResourcesForCounselors,
         empoweringKidsToHelpThemselves2,
         ...
         avoidLiabilityProblems1,
         consistentResourceLinks1,
         anonymityCounsellors,
         avoidDialoguesWebServices1,
         rememberLocationsOfResouces1,
         improveWebPostingTechnology2,
         considerationOfFeedbackITProviders1,
         completeResourceLinks2,
         performCounsellingAsInstructedBySupervisor,
         highDegreeOfSimilarityBetweenServicesProvided,
         helpAsManyKidsAsPossible3,
         engageInDialogWithCounselled,
         ...
         prideInJob,
         reducePhysicalObservationHearingByStaffNotInCounsellingTeam,
         ...
         avoidShortStaffing,
         acquirePublicSpeakingSkills3,
         improveCounsellingSkills2,
...
webCounsellingBePerformed1 <--
  [ performFirstReadingOfCounsellingPosts ].
...
accessibleResourcesForCounselors3 <--
  [ helps(iconsForCounsellingInformationBeOnDesktop) ].
accessibleResourcesForCounselors6 <--
  [ helps(provideResourcesInOneSource) ].
accountabilityOfServices2 <-- [helps(webCounsellingBePerformed)].
acquirePublicSpeakingSkills <-- [makes(undergoMediaTraining)].
acquireRecognizableEmploymentExperience <--
  [ helps(positiveEmploymentEvaluation) ].
avoidBeingDirective <-- [helps(aResolutionFocusApproachBeUsed)].
avoidBeingLabelledAntiTechnology <--
  [ hurts(commentOnTechnologyUsabilityIssues),
    helps(improveITSkills) ].
avoidBurnout <--
  [ helps(decreaseWorkLoad),
    helps(reducePrankCalls1),
    helps(enthusiasticWorkAtmosphere),
    helps(releaseTension),
    helps(decreaseWorkDifficulty) ].
avoidLiabilityProblems <--
  [ helps(controlOfCounsellingWork),
    helps(increasedCautionContentOfOnlineServices),
    helps(diffuseConflictWithParent2) ].
avoidPressureToPerformDutiesQuickly <-- [helps(performDutiesQuickly)].
avoidRelationshipsWithSpecificCounsellor1 <--
  [ helps(encourageKidsToTalkToDifferentCounsellors) ].
avoidShortStaffing <-- [helps(increaseNumberOfCounsellors)].
avoidTensionConfrontationAnimosityAmongstEmployees <--
  [ helps(professionalWorkEnvironment1),
    helps(counsellorsPoliceThemselves),
    hurts(counsellorsBeUnionized),
    helps(reducePhysicalObservationHearingByStaffNotInCounsellingTeam),
    helps(getRidOfUnion),
    helps(controlOfFloorOperations),
    helps(considerationOfFeedbackManagers),
    helps(reduceMonitoringOfActivities) ].
comfortableWithPerceivedPurposeOfTechnology <--
  [ helps(reduceMonitoringOfActivities) ].
completeResourceLinks4 <--
  [ helps(resourcesBeProvided),
    helps(updateOutdatedLinks) ].
considerationOfFeedbackITProviders <--
  [ helps(counsellorsBeUnionized) ].
considerationOfFeedbackManagers <--
  [ helps(counsellorsBeUnionized),
    hurts(getRidOfUnion),
    helps(beConsultedAboutNewPolicies) ].
consistentCounsellingResponses <--
  [ helps(highQualityResources1),
    helps(improveCounsellingSkills2) ].
consistentResourceLinks2 <--
  [ helps(provideResourcesInOneSource),
    hurts(provideResourcesAsCurrentlyDone),
    hurts(provideResourcesAsCurrentlyDone) ].
counsellorsEngageEmotionallyInSession <--
  [ helps(decreaseClumsinessTechnology2),
    hurts(webCounsellingBePerformed) ].
currentResourceLinks1 <-- [helps(reportOutdatedLinks1)].
currentResourceLinks4 <-- [helps(updateOutdatedLinks)].
decreaseDifficultiesWithTechnology <--
  [ helps(comfortableWithPerceivedPurposeOfTechnology),
    helps(improveWebPostingTechnology),
    helps(decreaseClumsinessTechnology),
    helps(considerationOfFeedbackITProviders),
    helps(simpleTechnology),
    helps(improveQualityAssuranceTechnology1),
    helps(commentOnTechnologyUsabilityIssues),
    helps(improveITSkills),
    helps(easilyAccessableTechnologyInstructions),
    helps(itSystemsBeIntegrated) ].
decreaseFactoryWorkerAtmosphere <--
  [ helps(avoidPressureToPerformDutiesQuickly),
    helps(considerationOfFeedbackITProviders),
    helps(beConsultedAboutNewPolicies),
    helps(avoidTensionConfrontationAnimosityAmongstEmployees),
    helps(considerationOfFeedbackManagers) ].
decreaseKidsIsolation <--
  [ helps(avoidDialoguesWebServices1),
    hurts(webCounsellingBePerformed),
    helps(encourageKidsToTalkToOthers) ].
decreaseWorkDifficulty <--
  [ helps(completeResourceLinks),
    helps(rememberLocationsOfResouces),
    helps(highDegreeOfSimilarityBetweenServicesProvided),
    helps(decreaseDifficultiesWithTechnology),
    helps(clearerCallClassificationCatagories),
    helps(currentResourceLinks),
    helps(accessibleResourcesForCounselors) ].
...
empoweringKidsToHelpThemselves2 <-- [helps(avoidBeingDirective)].
enthusiasticWorkAtmosphere <--
  [ helps(prideInJob),
    hurts(performDutiesQuickly),
    helps(highQualityCounselling1),
    hurts(professionalWorkEnvironment1),
    helps(recognition),
    helps(avoidTensionConfrontationAnimosityAmongstEmployees) ].
facilitateFasterChangesInCounsellorDuties1 <--
  [ hurts(counsellorsBeUnionized),
...
findEffectiveCues <--
  [ helps(listenForCues),
    hurts(webCounsellingBePerformed) ].
happinessCounsellors <--
  [ helps(implementEmailForCounsellors2),
    helps(avoidLiabilityProblems),
    helps(decreaseWorkDifficulty),
    helps(avoidTensionConfrontationAnimosityAmongstEmployees),
    helps(decreaseFactoryWorkerAtmosphere),
    helps(controlOfCounsellingWork),
    helps(reduceMonitoringOfActivities),
    helps(enthusiasticWorkAtmosphere),
    helps(increasedPeerSupport2),
    helps(releaseTension),
    helps(decreaseWorkLoad),
    helps(positiveEmploymentEvaluation),
    helps(varietyInWork),
    ...
    helps(avoidBurnout),
    helps(prideInJob),
    helps(accommodateSchedules),
    helps(reducePrankCalls1),
    helps(recognition),
    helps(safety) ].
helpAsManyKidsAsPossible <-- [helps(performDutiesQuickly)].
helpAsManyKidsAsPossible1 <--
  [ helps(webResponsesBeGeneric2),
    helps(voiceCounsellingBePerformed),
    helps(webCounsellingBePerformed) ].
helpKids <-- [helps(decreaseKidsIsolation)].
highDegreeOfSimilarityBetweenServicesProvided2 <--
  [ helps(provideWebCounsellingWithVideo),
    helps(provideWebCounsellingWithAudio) ].
highQualityCounselling <--
  [ helps(highQualityResources1),
    helps(counsellorsEngageEmotionallyInSession),
    helps(increaseConnectionBetweenCounsellorsAndCounselled),
    helps(avoidBeingDirective),
    helps(consistentCounsellingResponses),
    helps(increasedPeerSupport),
    helps(improveCounsellingSkills2),
    helps(accessibleResourcesForCounselors1),
    helps(supportIndividualityInCounsellingTechniques),
    helps(correctInterpretationOfCounsel),
    helps(findEffectiveCues),
    helps(empoweringKidsToHelpThemselves2),
    helps(decreaseKidsIsolation),
    helps(effectiveUseOfTechnologyCounsellors1) ].
highQualityCounselling1 <--
  [ helps(releaseTension),
    helps(happinessCounsellors) ].
highQualityResources <--
  [ helps(consistentResourceLinks),
    helps(currentResourceLinks1) ].
highQualityResources3 <--
  [ helps(consistentResourceLinks2),
    helps(completeResourceLinks4),
    helps(accessibleResourcesForCounselors6) ].
improveCommunicationBetweenCounsellors <--
  [ helps(implementEmailForCounsellors),
    helps(implementBulletinBoard),
    helps(provideSupportToCounsellors) ].
improveCounsellingSkills <--
  [ helps(increaseFeedback),
    helps(trainingBeReceived) ].
improveCounsellingSkills1 <--
  [ helps(enthusiasticWorkAtmosphere),
    helps(undergoClinicalReview1) ].
improveITSkills1 <-- [helps(increaseITTraining)].
improveQualityAssuranceTechnology <--
  [ helps(improveCallRecordingEquipment) ].
improveWebPostingTechnology1 <--
  [ helps(easierToFindPostsWebPostingTechnology),
    helps(cuttingAndPastingBeAddedToWebPostingSoftware),
    helps(reduceNumberOfStepsWebPostingTechnology) ].
increaseConnectionBetweenCounsellorsAndCounselled <--
  [ helps(engageInDialogWithCounselled),
    helps(deepenCounsellingSession) ].
increaseFeedback <--
  [ helps(increaseITMethodsToAcquireFeedback),
    helps(acquireFeedback),
    helps(increaseEmphasisOnOnlineFeedbackForm) ].
increasedCautionContentOfOnlineServices2 <--
  [ hurts(performEmailCounselling),
    hurts(reduceNumberOfStepsWebPostingTechnology),
    helps(moderateDiscussionBoards),
    helps(editOnlinePostsOfKids),
    helps(webResponsesBeGeneric),
    helps(blockKidsWhoDisplayInappropriateBehavoir),
    hurts(moderateAChat),
    helps(performFirstReadingOfCounsellingPosts),
    helps(removeInappropriateMaterial) ].
increasedPeerSupport1 <--
  [ helps(reduceStaggeringOfSchedules),
    helps(improveCommunicationBetweenCounsellors),
    helps(effectiveDebriefing) ].
moveOnToBetterJob <--
  [ hurts(happinessCounsellors),
    helps(acquireRecognizableEmploymentExperience) ].
...
performDutiesQuickly <--
  [ helps(decreaseDifficultiesWithTechnology),
    ...
    helps(decreaseClumsinessTechnology),
    helps(clearerCallClassificationCatagories),
    hurts(increasedCautionContentOfOnlineServices),
    helps(happinessCounsellors) ].
positiveEmploymentEvaluation <--
  [ helps(undergoClinicalReview1),
    helps(avoidBeingLabelledAntiTechnology),
    helps(performCounsellingAsInstructedBySupervisor),
    helps(improveITSkills),
    helps(improvedWritingSkills),
    helps(acquirePublicSpeakingSkills3),
    helps(improveCounsellingSkills1) ].
prideInJob <--
  [ helps(helpAsManyKidsAsPossible),
    helps(accountabilityOfServices),
    helps(highQualityCounselling1),
    helps(increaseCSAwareness) ].
professionalWorkEnvironment1 <--
  [ hurts(toolsToBurnNegativeEnergyBePresent),
    hurts(releaseTension) ].
recognition2 <--
  [ helps(acquirePublicSpeakingSkills1),
    helps(promoteCounselling),
    helps(participateInEvents1),
    helps(speakPubliclyAboutJob) ].
reduceContagionEffectOfHarmfulActions <--
  [ helps(editOnlinePostsOfKids),
    helps(moderateDiscussionBoards),
    helps(moderateAChat) ].
...
releaseTension <--
  [ helps(increasedPeerSupport2),
    helps(toolsToBurnNegativeEnergyBePresent) ].
retainJob <-- [helps(positiveEmploymentEvaluation)].
safety <-- [helps(anonymity)].
supportIndividualityInCounsellingTechniques <--
  [ hurts(performCounsellingAsInstructedBySupervisor1),
    hurts(webCounsellingBePerformed),
    helps(counsellorsEngageEmotionallyInSession) ].
toolsToBurnNegativeEnergyBePresent <--
  [ helps(avoidTensionConfrontationAnimosityAmongstEmployees) ].
...
webCounsellingBePerformed <--
  [ hurts(deepenCounsellingSession),
    hurts(engageInDialogWithCounselled) ].
