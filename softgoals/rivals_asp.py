#!/usr/bin/env python3
# rivals_asp.py : translate each softgoals model to ASP (clingo)
# and solve for the lexicographic optimum: max benefit (quals won)
# at priority 2, min footprint (leaves bought) at priority 1.
#
# Encoding (disclosed in the report):
# - clause h <- body: tseitin aux atoms for and/or nesting
# - (= x t) in a body -> literal x ; (= x f) -> literal "not x"
#   (stratified negation: demands only, bodies stay negation-free)
# - links: helps/makes from context C -> sup_q :- C.
#          hurts/breaks from context C -> att_q :- C.
#   quality rule: q :- sup_q, not att_q.   (deterministic
#   idealization of our 2:1 dice: support wins unless attacked)
# - leaves: choice {l}.   hard goals: :- not h.
# - cycles get stable-model semantics (unfounded loops = false),
#   UNLIKE our engine's optimistic memo. Disclosed.
