...
;;;;   or                -> a settled, delivered branch wins
;;;;                        outright; dice only otherwise
...
;;;; supplied t on a defined atom is a claim and must re-earn
;;;; (RE-EARN/MUST), else diy/t-with-no-coders sails through.
		     #+sbcl
(progn
...
(defun eager (xs w)
  ...
  (loop for x in xs
        if (every (lambda (a) (known a w)) (syms x)) collect x into yes
        else collect x into no
        finally (return (append yes (shuffled no)))))
...
(defun isamp (g w)
  (cond
    ...
    ((eq (car g) 'must) (every (lambda (x) (and (isamp x w)
                                                (believe x 't w)))
                               (cdr g)))
    ((eq (car g) 'and) (every (lambda (x) (isamp x w)) (eager (cdr g) w)))
    ((eq (car g) 'or)  (or (some (lambda (x) (settled x w)) (cdr g))
                           (let ((x (pick (cdr g))))   ; paid branch wins;
                             (and (isamp x w) (delivered x w)))))  ; else bet
    ((assoc (car g) *links*)
...
(defmodel gate :doc "or is a bet: picked branch must deliver"
  :hard (done)
  :rules ((<- done (or a b))
          (<- a (and hardwork))
          (<- b (and luck))))
