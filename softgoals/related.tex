...
iStar~2.0~\cite{dalpiaz16}, and GRL within the User
Requirements Notation~\cite{amyot10}.
...
results that rarely travel between notations.

Our niche in that map is \emph{simulation plus test-driven
minimization}. The minimization side imports a lineage RE has
barely used: ddmin~\cite{zeller02} and its modern
descendants~\cite{probdd24}, and---closest in spirit---%
...
modeling~\cite{gengoal25}, and sobering evaluations of
prompting limits~\cite{llmgoal26}. Extraction is plausible;
extraction \emph{alone} is of limited value---it produces
diagrams that humans must then read, and a 300-node
hallucination-prone diagram is not obviously better than the
