#!/usr/bin/env python3
# rivals_best.py : optimizer rivals vs shortr2 (nfr7.lisp), no
# minimization on the rival side -- these searches are slow, so
# each rival contributes only its BEST-FOUND full-leaf
# prescription (seed 1); shortr2 contributes its ddmin keys.
# Both are replayed 30x through the same truth walk (rig7.lisp
# rivals mode) and judged by cliffs/ks/cohen, win by lo then mu.
#   python3 rivals_best.py nsga2|smac|shortr1        # search+dump
#   python3 rivals_best.py replay RIVAL SEED         # one seed row
