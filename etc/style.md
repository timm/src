...
- An app may LAYER modules: a tiny core `xx.*` plus addon
  modules `xxplus.*` (cf. luamine's lib/luamine/lapps), each
  paired with its OWN `-eg` course (xx-eg, xxplus-eg). All
  share one manifest (`modules` lists both), one README, and
  the repo glossary -- still one app, one dir. An addon
  ...
  xaiplus (learners & optimizers layered on top).
